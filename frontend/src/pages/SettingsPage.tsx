          ...
          {activeSection === "ai" && (
            <div className="form-grid">
              <div className="field field-wide">
                ...
                <small>All configured models cooperate together. The highest-tier model handles primary reasoning.</small>
              </div>
              <div className="model-pool-list field-wide">
                {configQuery.data?.model_pool?.length ? (
                  configQuery.data.model_pool.map((entry, idx) => (
                    <div key={`${entry.model}-${idx}`} className={`model-pool-card ${entry.role === "primary" ? "model-primary" : ""} ${!entry.enabled ? "model-disabled" : ""}`}>
                      <div className="model-pool-header">
                        <span className={`model-role-badge role-${entry.role}`}>
                          {entry.role === "primary" ? "👑 Primary" : entry.role === "secondary" ? "🥈 Secondary" : entry.role === "fast" ? "⚡ Fast" : "🔄 Fallback"}
                        </span>
                        <span className="model-tier">Tier {entry.tier}</span>
                        <span className={`model-status ${entry.enabled ? "status-active" : "status-disabled"}`}>
                          {entry.enabled ? "● Active" : "○ Off"}
                        </span>
                      </div>
                      <div className="model-pool-body">
                        <strong>{entry.name}</strong>
                        <code>{entry.model}</code>
                      </div>
                      <div className="model-pool-footer">
                        <span className="model-provider-badge">{entry.provider.toUpperCase()}</span>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="empty-state">No models configured. Set API keys in .env to auto-detect models.</div>
                )}
              </div>
              <label className="field">
              ...
              <label className="field">
                ...
                <small>Primary model (auto-set from pool tier 1)</small>
              </label>
