...
import { getConfig, getConstraintAudit, getMcpDiagnostics, getModelPool, getReportContent, getReports, getSandboxSessions, getTarget, getTargetDiff, getTargetPreview, getTargetSnapshots, getTargets, getTasks } from "../api/web";
...
export function useModelPoolQuery() {
  return useQuery({ queryKey: ["model-pool"], queryFn: getModelPool });
}
...
export function useSandboxSessionsQuery() {
  return useQuery({
    queryKey: ["sandbox-sessions"],
    queryFn: getSandboxSessions,
    staleTime: 5_000,
    refetchInterval: 5_000,
  });
}
