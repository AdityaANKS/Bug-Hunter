...
type AppView = "home" | "risk" | "reports" | "boundary" | "history" | "settings" | "advanced" | "sandbox";
...
function buildViewMeta(t: TFunction): Record<AppView, { eyebrow: string; title: string; copy: string }> {
  return {
    ...
    sandbox: {
      ...
      copy: "Isolated Kali Linux environment for security tool execution",
    },
  };
}
...
export function App() {
  ...
  const nav = useMemo(
    () => [
      ...
      { key: "sandbox" as const, label: "Sandbox", description: "", icon: "/icons/sidebar/sandbox.svg" },
    ],
    [t],
  );
  ...
  const quickActions: ShellAction[] = useMemo(() => [
    ...
    { label: "Sandbox", glyph: "K", active: activeView === "sandbox", onClick: () => navigateToView("sandbox") },
    {
  ...
  return (
    ...
    >
      ...
      {activeView === "sandbox" && <SandboxPage />}
