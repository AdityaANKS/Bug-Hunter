...
async function requestJson<T>(input: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error("Unable to reach the BugHunter backend API. Start `bughunter web` and reconnect.");
  }
  ...
  try {
    ...
    throw new Error("The backend API returned non-JSON content. Confirm the backend was started with `bughunter web`.");
  }
}
...
export function getModelPool(): Promise<ModelPoolEntry[]> {
  return requestJson<ModelPoolEntry[]>("/api/models/pool");
}

export function updateModelPool(pool: ModelPoolEntry[]): Promise<ConfigView> {
  return requestJson<ConfigView>("/api/models/pool", {
    method: "POST",
...
export function getSandboxSessions(): Promise<SandboxSession[]> {
  return requestJson<SandboxSession[]>("/api/sandbox/status");
}

export function startSandbox(sessionId?: string): Promise<SandboxSession> {
  return requestJson<SandboxSession>("/api/sandbox/start", {
    method: "POST",
    body: JSON.stringify({ session_id: sessionId ?? null }),
  });
}

export function execSandboxCommand(
  sessionId: string,
  command: string,
  timeout = 30,
): Promise<SandboxCommandResult> {
  return requestJson<SandboxCommandResult>(`/api/sandbox/${sessionId}/exec`, {
    method: "POST",
    body: JSON.stringify({ command, timeout }),
  });
}

export function stopSandbox(sessionId: string): Promise<{ status: string; session_id: string }> {
  return requestJson(`/api/sandbox/${sessionId}/stop`, { method: "POST" });
}