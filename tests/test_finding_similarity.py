"""BugHunter Finding Similarity Tests — semantic deduplication module."""
...
class TestNormalizeText:
    ...
    def test_url_path_standardized(self):
        # Trailing slash removed,scheme/host Normalization
    ...
    def test_noise_tags_removed(self):
        out = normalize_text("[Automatic] SQLInjection [Confirmed]")
...
class TestUrlSimilarity:
    ...
    def test_same_path_different_query_value(self):
        # Same host/path/Parameter name, only different parameter value — Should be considered the same interface (=1.0)
    ...
    def test_same_path_different_query_keys(self):
        # host Same + path Same + Different Parameter Names(0) => 0.3 + 0.4 + 0 = 0.7
        a = "https://t.com/api/user?id=1"
        b = "https://t.com/api/user?name=bob"
        assert abs(url_similarity(a, b) - 0.7) < 1e-9

    def test_different_host(self):
        # host Different(0) + path Same + No parameters(1) => 0 + 0.4 + 0.3 = 0.7
    ...
    def test_non_url_fallback_to_text(self):
        # Non URL Fallback to Text Similarity
...
class TestNormalizeVulnType:
    def test_sqli_aliases(self):
        ...
        assert normalize_vuln_type("SQLInjection") == "sql_injection"
    ...
    def test_idor(self):
        ...
        assert normalize_vuln_type("Overstepping authority") == "insecure_direct_object_reference"
...
class TestFindingSimilarity:
    def test_same_vuln_different_wording(self):
        ...
        b = _mk(
            "login area SQL Injection",
            vuln_type="SQLInjection",
            description="https://t.com/api/login?u=2 Injection vulnerability",
        )
        # Type normalization matching 0.8*0.3 + URL Same Interface 1.0*0.4 + Description partially overlaps
...
class TestSessionStateIntegration:
    def test_semantic_dedup_on_add(self):
        ...
        # Semantic duplicates should be rejected (return False),findings Still only 1 Individual
