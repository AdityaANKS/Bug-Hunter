"""BugHunter Report Module Tests — generator.py + poc_builder.py"""
...
class TestReportGenerator:
    ...
    def test_report_includes_location_and_repro_details(self, tmp_path):
        ...
        finding = VulnerabilityFinding(
            ...
            severity="Critical",
            vuln_type="RCE",
            description="Verification confirmation through tools:admin The interface has command execution",
            evidence="https://example.com/admin/exec | /admin/exec | Confirm verification through tools: command execution successful",
        )
        finding.mark_verified(note="whoami Return www-data")
    ...
    def test_report_contains_bughunter_brand(self, tmp_path):
        from bughunter.report.generator import generate_report
    ...
    def test_report_prefers_llm_attack_summary_when_generated_from_session(
        ...
        session = self._make_session()
        monkeypatch.setattr(
            "bughunter.report.generator._generate_attack_summary_from_session",
            lambda session: "This is achieved through BugHunter Docking LLM Generated attack path summary.",
        )

        output = str(tmp_path / "report_llm_summary.md")
        generate_report(session, output)
        content = Path(output).read_text(encoding="utf-8")
        assert "This is achieved through BugHunter Docking LLM Generated attack path summary." in content

    def test_report_summary_uses_gpt5_token_parameter(self):
        from bughunter.config.schema import BugHunterConfig
        from bughunter.report.generator import _build_report_summary_llm_kwargs
    ...
    def test_generate_report_from_target_state_includes_governance_context(self, tmp_path):
        ...
        target_state = {
            ...
            "executed_steps": ["Round 1: Access /admin Failure"],
            "notes": [],
            "resume_meta": {
                "resume_strategy": "continue_scan",
                "resume_strategy_reason": "There are already high-value reconnaissance assets, continue candidate verification",
                ...
                "recent_failed_steps": ["Round 1: Access /admin Failure"],
            },
            "resume_summary": "Prioritize testing after recovery. /admin With vpn.example.com",
            "recon_meta": {
                "paths": {
    ...
    def test_persistent_cycle_report_prefers_llm_attack_summary(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "bughunter.report.generator._generate_attack_summary_from_session",
            lambda session: "From LLM Summary of the ongoing penetration cycle",
        )
        ...
        content = Path(output).read_text(encoding="utf-8")
        assert "From LLM Summary of the ongoing penetration cycle" in content
...
class TestPoCBuilder:
    ...
    def test_generate_single_poc(self):
        from bughunter.report.poc_builder import generate_single_poc
    ...
    def test_generate_pocs_extracts_target_from_evidence(self, tmp_path):
        ...
        session.add_finding(
            VulnerabilityFinding(
                ...
                severity="High",
                vuln_type="LFI",
                evidence="Accessible address https://victim.local/download?file=../../etc/passwd And return root:x:0:0",
            )
        )
