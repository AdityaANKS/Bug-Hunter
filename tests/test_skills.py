"""BugHunter Skill Module Tests — loader.py + dispatcher.py"""
...
class TestSkillLoader:
    ...
    def test_list_core_skills(self):
        from bughunter.skills.loader import list_core_skills
    ...
    def test_list_specialized_skills(self):
        from bughunter.skills.loader import list_specialized_skills
    ...
    def test_load_core_skill(self):
        from bughunter.skills.loader import load_core_skill

        skill = load_core_skill("pentest-flow")
        assert skill is not None
        assert "content" in skill
        assert "Penetration" in skill["content"]
    ...
    def test_load_secknowledge_reference(self):
        """SecKnowledge references should be loadable through BugHunter."""
        from bughunter.skills.loader import load_skill_reference

        content = load_skill_reference("secknowledge-skill", "bughunter-ctf-src-routing.md")
        assert content is not None
    ...
    def test_all_specialized_skills_loadable(self):
        ...
        from bughunter.skills.loader import list_specialized_skills, load_skill_by_name
    ...
    def test_all_core_skills_loadable(self):
        ...
        from bughunter.skills.loader import list_core_skills, load_skill_by_name
...
class TestSkillDispatcher:
    ...
    def test_dispatch_pentest_flow(self):
        ...
        skill = d.dispatch("Penetration testing on the target")
        assert skill["name"] == "pentest-flow"
    ...
    def test_dispatch_specialized_over_core(self):
        ...
        d = self._make_dispatcher()
        # "Intranet" could match post-exploitation (core) or intranet-pentest-advanced (specialized)
        # With the 1.5x boost, specialized should win for specific intranet keywords
        skill = d.dispatch("Intranet penetration lateral movement")
        assert skill["name"] == "intranet-pentest-advanced"

    def test_dispatch_case_insensitive(self):
        """Dispatch should be case-insensitive."""
        d = self._make_dispatcher()
        skill1 = d.dispatch("SQLInjection")
        skill2 = d.dispatch("sqlInjection")
    ...
    def test_dispatch_crypto_toolkit(self):
        ...
        d = self._make_dispatcher()
        skill = d.dispatch("Help mebase64Decode")
        assert skill["name"] == "crypto-toolkit"

    def test_dispatch_crypto_hash(self):
        d = self._make_dispatcher()
        skill = d.dispatch("MD5Hash encryption")
        assert skill["name"] == "crypto-toolkit"
...
class TestCryptoTools:
    ...
    def test_list_operations(self):
        from bughunter.skills.crypto_tools import list_operations
