...
async def test_runtime_executes_builtin_plugin():
    runtime = create_builtin_runtime(BugHunterConfig())
...
def test_plugin_finding_converts_to_vuln_finding():
    from bughunter.plugins.integration import plugin_finding_to_vuln_finding
    from bughunter.plugins.result import PluginFinding, RiskLevel
    ...
    pf = PluginFinding(
        ...
        remediation="Complete the response headers",
        confidence=0.9,
    )
...
def test_merge_plugin_results_dedups_into_session():
    ...
    assert added_again == 0  # Deduplication: identical finding Not Counted Again
