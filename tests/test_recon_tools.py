...
def test_extract_from_js_infers_rest_crud_from_base_and_entity():
    """base path + entity name → CRUD Verb inference (fix User/list、Org/list Missed judgments)."""
    content = """
...
def test_extract_from_js_discovers_dynamic_entities():
    """PascalCase Dynamic extraction of entity names——Not relying on hardcoded entity lists."""
    content = """
        var base = "/smweb/rest";
        url: "AppRoleService";
        var c = "Count";
...
def test_extract_from_js_captures_short_fragments():
    """Not based on / Starting with REST Fragment (such as "User/list") also needs to be extracted."""
...
def test_extract_from_js_captures_framework_verb_variants():
    """listForLayUI / getAllByType Variants of the framework also match."""
    content = """$.post(base + "Article/listForLayUI", data);"""
    out = recon_tools.extract_from_js(content)
    assert any("Article/listForLayUI" in p for p in out["paths"])


# ── Spatial mapping: Query construction + Analysis ───────────────────────────────────────
...
# ── Directory enumeration: Global masquerade 200 Identification ─────────────────────────────────────


async def test_dir_enum_aborts_on_global_200(monkeypatch):
    def router(method, url, params, content):
        # Any path returns 200 → Camouflage
...
# ── Subdomain enumeration: passive aggregation + Do not parse when dictionary brute force is closed ─────────────────────
...
async def test_unauth_test_skips_destructive_and_flags_data(monkeypatch):
    def router(method, url, params, content):
        ...
        if url.endswith("/api/user/profile"):
            return _Resp(text="<html>Please log in</html>", status=200)
        return _Resp(text="nope", status=404)
...
async def test_js_recon_auto_probes_endpoints(monkeypatch):
    ...
    assert "Unauthorized access detection" in res
