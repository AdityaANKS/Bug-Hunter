"""BugHunter MCP Module Tests — registry.py + router.py + lifecycle.py"""
...
class TestMCPRegistry:
    ...
    def test_register_server(self):
        from bughunter.mcp.registry import MCPRegistry
...
class TestMCPRouter:
    ...
    def test_route_fetch(self):
        from bughunter.mcp.router import MCPRouter

        router = MCPRouter()
        results = router.route("Send a request to access this interface")
    ...
    def test_route_burp(self):
        ...
        router = MCPRouter()
        results = router.route("Help me capture packets to look at this request")
        assert len(results) > 0
    ...
    def test_route_browser(self):
        ...
        router = MCPRouter()
        results = router.route("Open the webpage to take a look")
        assert len(results) > 0
    ...
    def test_route_memory_save(self):
        ...
        router = MCPRouter()
        results = router.route("Remember this discovery")
        assert len(results) > 0
    ...
    def test_route_no_match(self):
        ...
        router = MCPRouter()
        results = router.route("How's the weather today?")
        assert len(results) == 0
    ...
    def test_extract_url(self):
        ...
        assert router.extract_url("NoneURL") is None
    ...
    def test_extract_ip(self):
        ...
        assert router.extract_ip("NoneIP") is None

    def test_suggest_tools_for_phase(self):
        from bughunter.mcp.router import MCPRouter

        router = MCPRouter()
        tools = router.suggest_tools_for_phase("Information gathering")
        assert len(tools) > 0
...
class TestStructuredToolResults:
    ...
    async def test_tool_call_manager_preserves_structured_content(self):
        from bughunter.agent.tool_call_manager import handle_tool_calls_with_results
