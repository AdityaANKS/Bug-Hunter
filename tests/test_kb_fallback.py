...
import bughunter.kb.retriever as retriever_mod
...
def _seed_store(tmp_path):
    ...
    store.add_entry(
        ...
        "sqli-bypass",
        {
            ...
            "description": "Bypass WAF 's SQL injection payload Construction method",
    ...
    store.add_entry(
        ...
        "xss-bypass",
        {
            "title": "XSS Bypassing techniques",
            "description": "Bypass WAF 's cross site scripting payload",
...
class TestContextCaching:
    def test_same_query_uses_cache(self, tmp_path, monkeypatch):
        monkeypatch.setattr(retriever_mod, "CHROMADB_AVAILABLE", False)
        from bughunter.agent import kb_context as kbc
