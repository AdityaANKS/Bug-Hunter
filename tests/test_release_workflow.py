...
def test_verify_dist_artifacts_accepts_matching_version(tmp_path) -> None:
    ...
    wheel = tmp_path / "bughunter-0.3.2-py3-none-any.whl"
    sdist = tmp_path / "bughunter-0.3.2.tar.gz"
...
def test_verify_dist_artifacts_rejects_missing_files(tmp_path) -> None:
    ...
    (tmp_path / "bughunter-0.3.2-py3-none-any.whl").write_bytes(b"wheel")
...
def test_verify_dist_artifacts_rejects_empty_files(tmp_path) -> None:
    ...
    (tmp_path / "bughunter-0.3.2-py3-none-any.whl").write_bytes(b"")
    (tmp_path / "bughunter-0.3.2.tar.gz").write_bytes(b"sdist")
