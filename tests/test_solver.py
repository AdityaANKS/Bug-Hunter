...
from bughunter.agent import solver
from bughunter.agent.blackboard import Blackboard, IntentStatus
...
def test_flag_evidence_helpers():
    ...
    # Claimed flag Not in the evidence → Determination unverified
    assert solver._unverified_flags("flag{fake}", "server said: error") == ["flag{fake}"]
    # In the evidence → Considered Verified
    assert solver._unverified_flags("flag{real}", "body: flag{real}") == []
    # flag Target but no evidence flag → Completion Not Established
    ...
    assert ok2 is True
    # Non flag Objective → Not mandatory
...
async def test_solve_completes_when_reason_signals_goal(monkeypatch):
    ...
    async def fake_explore(agent, board, intent, *, max_tool_rounds, evidence_buffer, stream_sink=None):
        # Simulate that the real tool output has appeared flag(The evidence gate is released based on this)
        evidence_buffer.append("HTTP 200\n<html>... flag{captured} ...</html>")
        return True, "sqli Confirm, extracted to flag{captured}"
...
async def test_solve_completes_immediately_on_verified_flag(monkeypatch):
    """Explore once real evidence is obtained flag Complete immediately, no need for additional validation rounds."""
    reason_calls = {"n": 0}
    ...
    async def fake_explore(agent, board, intent, *, max_tool_rounds, evidence_buffer, stream_sink=None):
        ...
        return True, "union Injection feedback flag{real_one}"
    ...
    # Get flag Immediate convergence:reason Called only once, no longer entering the next round of verification
    assert reason_calls["n"] == 1


async def test_solve_rejects_hallucinated_flag(monkeypatch):
    """Conclusion Claims flag Not in the real tool output → Determine hallucination、Deny、Incomplete."""
    reason_calls = {"n": 0}
    ...
    async def fake_reason(agent, board, max_intents):
        ...
        return {}  # No longer raised after → Frontier exhaustion

    async def fake_explore(agent, board, intent, *, max_tool_rounds, evidence_buffer, stream_sink=None):
        # Do not go evidence_buffer Write flag —— Simulated model fabrication out of thin air
    ...
    # A flag of denial generates a [Unverified] Facts
...
async def test_solve_rejects_ungrounded_completion(monkeypatch):
    """The target needs to flagBut the real tool output has never appeared flag → Deny Reason Completion statement of."""
    reason_calls = {"n": 0}

    async def fake_reason(agent, board, max_intents):
        reason_calls["n"] += 1
        if reason_calls["n"] == 1:
            return {"complete": "I think I have already obtained flag Completed"}  # No evidence to support
        return {}  # No longer raised after → Frontier exhaustion
...
async def test_solve_rejects_negated_completion_claim(monkeypatch):
    """The model treats「Not achieved」Write into complete Field → Must never misjudge achievement (reproducibility i004 False report)."""
    reason_calls = {"n": 0}

    async def fake_reason(agent, board, max_intents):
        reason_calls["n"] += 1
        if reason_calls["n"] == 1:
            # complete=true But reason Is a negation conclusion (should be intercepted by a negation gate)
            return {
                "complete": True,
                "reason": "f001 Only confirmed ports and fingerprints, goal not achieved",
                "evidence": ["f001"],
            }
        return {"complete": False}  # No longer raised after → Frontier exhaustion
    ...
    result = await solver.solve(
        _fake_agent(),
        origin="t",
        goal="Penetration analysis site",  # Non flag Target, old logic without any gate may misjudge achievement
        max_steps=10,
        on_event=lambda kind, payload: events.append(kind),
    )
...
async def test_solve_rejects_completion_without_explicit_bool(monkeypatch):
    """Old-fashioned {"complete": "<Text>"}(Non-explicit true) All handled as unaccomplished."""

    async def fake_reason(agent, board, max_intents):
        return {"complete": "I believe I have finished analyzing"}
    ...
    result = await solver.solve(
        _fake_agent(),
        origin="t",
        goal="Penetration analysis site",
        max_steps=10,
        on_event=lambda kind, payload: events.append(kind),
    )
...
async def test_solve_completes_nonflag_goal_with_evidence(monkeypatch):
    """Non flag Target:complete=true + No grounds for denial + Referencing real fact → Normally achieved."""

    async def fake_reason(agent, board, max_intents):
        return {
            "complete": True,
            "reason": "f001 Unauthorized access interfaces have been confirmed to exist, target achieved",
            "evidence": ["f001"],
        }
...
async def test_solve_abandons_unproductive_intent(monkeypatch):
    ...
    async def fake_explore(agent, board, intent, *, max_tool_rounds, evidence_buffer, stream_sink=None):
        return False, "This direction is not feasible"
    ...
    assert board.intents[0].note == "This direction is not feasible"
