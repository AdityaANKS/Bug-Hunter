...
class TestWebServices:
    def test_constraint_audit_service_aggregates_events(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("bughunter.web.services.constraint_audit_service.TARGETS_DIR", tmp_path)
    ...
    def test_web_config_service(self):
        from bughunter.web.services.config_service import get_public_config
    ...
    def test_web_config_service_updates_subset(self, monkeypatch, tmp_path):
        import bughunter.web.services.config_service as config_service
        from bughunter.config.schema import BugHunterConfig
        from bughunter.web.schemas import ConfigUpdateRequest

        saved = BugHunterConfig()
    ...
    def test_web_target_service_lists_targets(self, monkeypatch, tmp_path):
        import bughunter.target_state.store as store_mod
        import bughunter.web.services.target_service as target_service
    ...
    def test_web_target_service_preview_and_diff(self, monkeypatch, tmp_path):
        import bughunter.target_state.store as store_mod
        import bughunter.web.services.target_service as target_service
        from bughunter.agent.context import SessionState, TaskConstraints, VulnerabilityFinding
    ...
    def test_web_report_service_generates_target_report(self, monkeypatch, tmp_path):
        import bughunter.target_state.store as store_mod
        import bughunter.web.services.report_service as report_service
        from bughunter.agent.context import SessionState, VulnerabilityFinding
    ...
    def test_web_task_prompt_includes_explicit_constraints(self):
        from bughunter.web.schemas import TaskCreateRequest, TaskOptions
        from bughunter.web.services.task_service import _build_prompt_v2
    ...
    def test_web_task_options_reject_invalid_only_port(self):
        ...
        from bughunter.web.schemas import TaskOptions
    ...
    def test_web_task_manager_persists_and_restores_tasks(self, monkeypatch, tmp_path):
        import bughunter.web.task_manager as task_manager_mod
        from bughunter.web.schemas import TaskCreateRequest
    ...
    async def test_web_task_service_restore_summary_flow(self, monkeypatch):
        import bughunter.web.services.task_service as task_service
        from bughunter.agent.context import SessionState
        from bughunter.config.schema import BugHunterConfig
        from bughunter.web.schemas import TaskCreateRequest
        from bughunter.web.task_manager import WebTaskManager
    ...
    async def test_web_task_service_blocks_run_when_allowed_actions_conflict(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "_build_prompt_v2",
            lambda request: (
                "Perform authorized reconnaissance against https://example.com. Only collect information."
            ),
        )
    ...
    async def test_orchestrator_shared_run_flow(self, monkeypatch):
        import bughunter.orchestrator as orchestrator
        from bughunter.agent.context import SessionState
    ...
    def test_web_stream_encode(self):
        from bughunter.web.schemas import TaskEvent
...
class TestWebApp:
    def test_constraint_audit_route_works_without_fastapi(self, monkeypatch):
        import bughunter.web.app as web_app
    ...
    def test_frontend_toc_navigation_hides_advanced_console(self):
        ...
        assert "Unable to reach the BugHunter backend API" in api_source
        ...
        assert "Original JSON" not in risk_source
        assert "Original Target State" not in risk_source
    ...
    def test_static_fallback_is_toc_shell(self):
        ...
        source = (root / "bughunter" / "web" / "static" / "index.html").read_text(
            encoding="utf-8"
        )
