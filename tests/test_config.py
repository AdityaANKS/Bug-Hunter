"""BugHunter Config Module Tests — schema.py + settings.py"""
...
class TestMCPServerConfig:
    ...
    def test_default_values(self):
        from bughunter.config.schema import MCPServerConfig, MCPTransportConfig
...
class TestSessionConfig:
    ...
    def test_runtime_integration_default_values(self):
        from bughunter.config.schema import SessionConfig
...
class TestBugHunterConfig:
    """Test BugHunterConfig schema."""
    ...
    def test_mcp_builtin_servers(self):
        from bughunter.config.schema import BUILTIN_MCP_SERVERS, BugHunterConfig
    ...
    def test_builtin_mcp_server_count(self):
        from bughunter.config.schema import BUILTIN_MCP_SERVERS
    ...
    def test_provider_presets(self):
        from bughunter.config.schema import PROVIDER_PRESETS
    ...
    def test_llm_provider_enum(self):
        from bughunter.config.schema import LLMProvider
...
class TestSettingsLoad:
    ...
    def test_load_config_returns_config(self):
        ...
        config = load_config()
        assert isinstance(config, BugHunterConfig)
    ...
    def test_save_config(self):
        from bughunter.config.schema import BugHunterConfig
        from bughunter.config.settings import save_config
    ...
    def test_set_config_value(self):
        from bughunter.config.settings import set_config_value
    ...
    def test_set_config_mcp_server_field(self):
        from bughunter.config.settings import load_config, set_config_value
    ...
    def test_apply_provider_preset(self):
        from bughunter.config.schema import BugHunterConfig
        from bughunter.config.settings import apply_provider_preset
    ...
    def test_list_providers(self):
        from bughunter.config.settings import list_providers
    ...
    def test_env_var_override(self, monkeypatch):
        ...
        monkeypatch.setenv("BUGHUNTER_LLM_API_KEY", "env-test-key")
        monkeypatch.setenv("BUGHUNTER_LLM_MODEL", "env-test-model")
    ...
    def test_openai_default_headers_allow_user_agent_override(self, monkeypatch):
        from bughunter.config.settings import openai_default_headers

        assert openai_default_headers()["User-Agent"] == "Mozilla/5.0"

        monkeypatch.setenv("BUGHUNTER_LLM_USER_AGENT", "test-agent")

        assert openai_default_headers()["User-Agent"] == "test-agent"

    def test_env_var_override_new_session_fields(self, monkeypatch):
        """New additions from secondary development session Configuration (reflection/plugins) can be injected through environment variables."""
        from bughunter.config.settings import load_config

        monkeypatch.setenv("BUGHUNTER_SESSION_REFLEXION_ENABLED", "false")
        monkeypatch.setenv("BUGHUNTER_SESSION_REASONING_STATE_ENABLED", "false")
        monkeypatch.setenv("BUGHUNTER_SESSION_REFLEXION_MAX_SAME_VULN_FAILS", "5")
        monkeypatch.setenv("BUGHUNTER_SESSION_ESCALATION_MAX_LEVEL", "2")
        monkeypatch.setenv("BUGHUNTER_SESSION_PLUGIN_RUNTIME_ENABLED", "false")
        monkeypatch.setenv("BUGHUNTER_SESSION_PLUGIN_MAX_REQUESTS_PER_TARGET", "7")
        monkeypatch.setenv("BUGHUNTER_SESSION_EVIDENCE_MIN_REPORT_LEVEL", "L2")
