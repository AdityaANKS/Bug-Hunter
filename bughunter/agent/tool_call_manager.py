...
async def handle_tool_calls(agent: Any, message: Any) -> str:
    ...
    results: list[str] = []
    # [Modified] 2026-06-10 Nyaecho - Fix tool_calls attribute access issue, use getattr to prevent AttributeError
...
async def handle_tool_calls_with_results(
    ...
    if total_count > dedup_count:
        skipped_info.append(f"[Deduplicated] {total_count - dedup_count}  duplicate calls merged")
    if skipped_calls:
        for sc in skipped_calls:
            skipped_info.append(
                f"[Skipped] {sc['func_name']}({str(sc['func_args'])[:100]}) — This round reached limit, continuing next round"
            )
...
async def _execute_single(agent: Any, item: dict[str, Any]) -> dict[str, Any] | None:
    """Execute one tool call with isolated error handling and timeout.
    ...
    """
    _TOOL_TIMEOUT = 180  # seconds — hard cap per tool call
    ...
    try:
        ...
        return {
            "tool_call": tool_call,
            "tool_call_id": tool_call.id,
            "content": f"[tool:{func_name}] [!] Timed out after {_TOOL_TIMEOUT}s — target may be slow or tool scope too large.",
