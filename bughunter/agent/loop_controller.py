...
from bughunter.agent.llm_client import call_llm_auto
from bughunter.agent.reasoning_state import (
    ConstraintCategory,
    ConstraintSeverity,
    PathStatus,
)
...
def _sync_reasoning_path(agent: Any, path_name: str, *, success: bool) -> None:
    """the identified attackPathfall into structureReasoningStatus, and refresh the priority."""
    ...
    if success:
        ...
        existing.result = "get along with"
...
def _sync_reasoning_constraint(agent: Any, path_name: str, category: FailureCategory) -> None:
    """Put the identified obstacles (WAF/Filteretc.) fall into structuredReasoningStatus,DescriptionKeep it steady to lose weight."""
    ...
    reasoning.add_constraint(
        description=f"{path_name or 'currentPath'} quilt {category_value.value} block",
...
async def auto_pentest(
    ...
    *,
    ...
    # ── Time-budgeted scan setup ──────────────────────────────────
    session_cfg = getattr(agent.config, "session", None)
    budget_minutes = time_budget_minutes or getattr(session_cfg, "scan_time_budget", 0)
    scan_budget: Optional[ScanBudget] = None
    if budget_minutes > 0:
        scan_budget = ScanBudget(
            total_minutes=budget_minutes,
            tool_timeout_quick=getattr(session_cfg, "scan_tool_timeout_quick", 30),
            tool_timeout_standard=getattr(session_cfg, "scan_tool_timeout_standard", 120),
            tool_timeout_deep=getattr(session_cfg, "scan_tool_timeout_deep", 180),
        )
        scan_budget.start()
        _log.info("Scan budget: %d minutes, phases: quick→standard→deep→report", budget_minutes)
    # Expose budget on the runtime so tool executors can read it
    agent.runtime.scan_budget = scan_budget

    for round_num in range(1, max_rounds + 1):
        # ── Budget checks ─────────────────────────────────────────
        if scan_budget is not None:
            # Check if we need to advance the phase
            if scan_budget.check_phase_advance():
                _log.info(
                    "Budget phase advanced to %s (%.1f min remaining)",
                    scan_budget.phase.value,
                    scan_budget.remaining_minutes,
                )
            # Check if total budget is expired
            if scan_budget.is_expired():
                _log.info("Scan budget expired after %d rounds", round_num - 1)
                break
        ...
        # Inject time budget context into the round prompt
        if scan_budget is not None:
            round_context += scan_budget.to_prompt_context()
        ...
        try:
            ...
            # attackPathStructuring in step with obstaclesReasoningStatus
            ...
            if not has_new_progress and not agent.runtime.path_switch_forced:
                if detected_path:
                    if detected_path == agent.runtime.current_attack_path:
                        ...
                        # Switched attack after getting stuckPath = onceReflectionEvents, logged to close the upgrade loop
                        if (
                            reflexion_on
    ...
    # Put this bookcycleReflectionmemory write back SessionState Snapshot for nextcycle/sameTargetReuse during recovery
...
async def persistent_pentest(
    ...
    *,
    # stream_sink Depend on core.py pass in, pass on agent.auto_pentest() Implement streamingOutput
    stream_sink: Any = None,
    ...
    while not should_stop:
        ...
        try:
            ...
            results = await agent.auto_pentest(
                user_input=(
                    f"[Persistent Cycle {cycle_num}] continue toTarget {agent.context.state.target or 'unknown'} conductPenetration Testing."
                    f"This isStep {cycle_num} indivualcycle, keeping all previousFindingGo deeper."
                    f"{constraints_block}"
                    if cycle_num > 1
                    else user_input
                ),
                ...
                # pass-through stream_sink,make persistent Mode also supports streamingOutput
        ...
        if auto_report:
            try:
                from bughunter.report.generator import generate_persistent_cycle_report
                ...
                report_path = f"Report GenerationFailed: {e}"
