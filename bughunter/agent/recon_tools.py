...
- space_search      Unified spatial mapping (FOFA / Hunter / Quake / Shodan / ZoomEye / 0.zone)
- subdomain_enum    Subdomain enumeration (Spatial mapping passive aggregation + Optional small dictionary DNS blasting)
- js_recon          JS Reconnaissance (Reference URLFinder:Grasp JS Raise the endpoint/domain name/Key)
- dir_enum          Directory Enumeration (Reference dirsearch: Concurrent dictionary blasting + 404 baseline/Disguise identification)

Design principle: Passive first、strictly adhere to host/path/port constraint、All outbound callsTimeoutand concurrency upper limit、
Never hard in the source codeEncodingany API key(from config.recon or environment variable reading).
"""
...
from bughunter.agent.builtin_tools import enforce_host_path_constraints

# ── Built-in directory dictionary (compact version;config.recon.dir_wordlist_path Can be overridden as a large dictionary)────────
_BUILTIN_DIR_WORDLIST: tuple[str, ...] = (
    ...
    # Configuration / Debug / InfoGive way
    ...
    # Common business (MediumEnglish mixed pinyin)
    "user", "users", "member", "hy", "order", "dd", "pay", "payment", "list", "index",
    "home", "main", "portal", "wx", "mp", "xcx", "miniprogram", "h5", "mobile",
)

# ── Endpoint extraction regular (Reference URLFinder)──────────────────────────────────────────────
_URL_RE = re.compile(r"""https?://[a-zA-Z0-9.\-]+(?::\d+)?(?:/[^\s"'`<>()\\{}|^]*)?""")
# broadPathExtract: Any quotes within / beginning、Contains 2+ paragraphPath(Reference URLFinder wide matching strategy)
_PATH_RE = re.compile(
    r"""(?P<q>["'`])(?P<v>/[a-zA-Z0-9_\-]+/[a-zA-Z0-9_\-./?=&%]*)(?P=q)""",
    re.IGNORECASE,
)
# Short snippet extraction: not with / beginning but looks like REST A quoted string within the endpoint (e.g. "User/list")
# after verbAllowand ForXxx / All / ById and other framework variants (listForLayUI、getAllByType...)
_FRAG_RE = re.compile(
...
# CRUD verb template——and base path and JS MediumArrangements and combinations of entity names that appear
_CRUD_VERBS = ("list", "get", "save", "add", "delete", "update", "detail", "query",
               "info", "export", "tree", "page", "count", "all", "search")
# Dynamic entity name extraction: from JS MediumFind all PascalCase Identifier (first letter capitalized)、2+ letter),
# rather than hardEncodingEntity list——Any business entity can be captured
_PASCAL_CASE_RE = re.compile(
    r"""(?P<q>["'`])(?P<v>[A-Z][a-zA-Z0-9]{1,30}(?:[A-Z][a-zA-Z0-9]*)*)(?P=q)"""
)

# sensitiveInfo / Credentials leakedFingerprint
...
def _get_recon_cfg(agent: Any) -> Any:
    from bughunter.config.schema import ReconConfig
...
async def _engine_fofa(client: Any, query: str, size: int, cfg: Any) -> tuple[list[dict], str]:
    if not (cfg.fofa_email and cfg.fofa_key):
        return [], "fofa: not yetConfiguration fofa_email/fofa_key"
...
async def _engine_hunter(client: Any, query: str, size: int, cfg: Any) -> tuple[list[dict], str]:
    ...
    params = {
        ...
        "search": _b64(query),
        "page": "1",
        # Hunter only accept [10,100] of page_size, too small will be reported「Page size is illegal」
    ...
    return recs, f"hunter: {(data.get('data') or {}).get('total', len(recs))} command"


async def _engine_quake(client: Any, query: str, size: int, cfg: Any) -> tuple[list[dict], str]:
    if not cfg.quake_key:
        return [], "quake: not yetConfiguration quake_key"
    ...
    return recs, f"quake: {(data.get('meta') or {}).get('pagination', {}).get('total', len(recs))} command"


async def _engine_shodan(client: Any, query: str, size: int, cfg: Any) -> tuple[list[dict], str]:
    if not cfg.shodan_key:
        return [], "shodan: not yetConfiguration shodan_key"
    r = await client.get(
...
async def _engine_zerozone(client: Any, query: str, size: int, cfg: Any) -> tuple[list[dict], str]:
    ...
    return recs, f"zerozone: {data.get('total', len(recs))} command"
...
# only given domain , the default query syntax of each engine
_DOMAIN_QUERY = {
...
async def execute_space_search(agent: Any, args: dict[str, Any]) -> str:
    """Unified spatial mapping query.engine ∈ {fofa,hunter,quake,shodan,zoomeye,zerozone,all}."""
    ...
    if not query and not domain:
        return "[!] space_search need query or domain parameter"

    engines = list(_ENGINES) if engine == "all" else [engine]
    invalid = [e for e in engines if e not in _ENGINES]
    if invalid:
        return f"[!] Not supported engine: {', '.join(invalid)};optional: {', '.join(_ENGINES)}, all"

    out: list[str] = [f"# spatial mapping — {'/'.join(engines)}  query={query or domain}"]
    try:
        async with _make_client(cfg) as client:
            async def run(eng: str) -> tuple[str, list[dict], str]:
                ...
                except Exception as e:  # single engineFailedNoImpactother engines
                    return eng, [], f"{eng}: Request exception {e}"

            results = await asyncio.gather(*(run(e) for e in engines))
    except Exception as e:
        return f"[!] space_search implementError: {e}"
...
async def execute_subdomain_enum(agent: Any, args: dict[str, Any]) -> str:
    """Subdomain enumeration: Passive aggregation of spatial mapping + Optional small dictionary DNS blasting."""
    cfg = _get_recon_cfg(agent)
    domain = str(args.get("domain", "") or "").strip().lower()
    if not domain:
        return "[!] subdomain_enum need domain parameter"
    if "://" in domain:
    ...
    # 1) Passive: aggregated from various spatial mapping engines
            ...
            async with _make_client(cfg) as client:
                async def run(eng: str):
                    ...
                    try:
                        ...
                        notes.append(f"{eng}: abnormal {e}")
                await asyncio.gather(*(run(e) for e in engines))
        except Exception as e:
            notes.append(f"Passive aggregation exception: {e}")
    else:
        notes.append("not yetConfigurationany spatial mapping key,Skippedpassive aggregation")

    # 2) Active: small dictionary DNS Analytical blasting
    if do_brute:
        ...
        notes.append(f"DNS Explosive Dictionary {len(_SUBDOMAIN_BRUTE)} strip")

    subs = sorted(found)
    head = [f"# Subdomain enumeration — {domain}  common {len(subs)} indivual", "  " + "; ".join(notes)]
    return "\n".join(head + [f"  {s}" for s in subs]) if subs else "\n".join(head + ["  (not yetFindingsubdomain)"])
...
# ── JS Reconnaissance(Reference URLFinder)──────────────────────────────────────────────


def extract_from_js(content: str, base_host: str = "") -> dict[str, list[str]]:
    """from HTML/JS textMediumextract urls / paths / domains / secretsError 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know.

    Key improvements (Reference URLFinder):
    1. broadPathmatch——within any quotation marks /xxx/yyy Extract all, no keyword whitelist restrictions
    2. Short fragment extraction——"User/list" This kind of thing is not / beginning CRUD Fragments also capture
    3. base path + Entity name + CRUD Verb arrangement and combination inference——even if JS only appears in "/jalis/rest"
       and "User", can also be automatically inferred /jalis/rest/User/list Wait for the implicit endpoint
    """
    urls = _URL_RE.findall(content)
    paths = [m.group("v") for m in _PATH_RE.finditer(content)]

    # short snippets (e.g. "User/list")
    frags = [m.group("v") for m in _FRAG_RE.finditer(content)]

    # base path Extract (such as "/jalis/rest"、"/smweb/rest")
    bases = list(dict.fromkeys(m.group("v").rstrip("/") for m in _BASE_PATH_RE.finditer(content)))

    # Entity name dynamic extraction: from JS MediumFind all PascalCase identifier (Excludecommon JS Keywords/class name noise)
    _JS_NOISE = frozenset({
...
async def execute_js_recon(agent: Any, args: dict[str, Any]) -> str:
    """crawlTargetPages and their references JS file, extraction endpoint / domain name / Key."""
    cfg = _get_recon_cfg(agent)
    url = str(args.get("url", "") or "").strip()
    if not url:
        return "[!] js_recon need url parameter"
    if "://" not in url:
        url = "http://" + url
    host = _host_of(url)
    ...
    try:
        async with _make_client(cfg) as client:
            ...
            # collect <script src> and complete to absolute URL
            js_urls = []
        ...
        return f"[!] js_recon implementError: {e}"

    for k in agg:
        agg[k] = _dedup_cap(agg[k], 200 if k != "secrets" else 50)

    out = [f"# JS Reconnaissance — {url}  (crawl {fetched} indivual JS)"]

    # keyFindingAhead: SensitiveInfoand unauthorized detectionResultPut it at the front and reduce it after being truncated LLM Repeatedly reset
    if agg["secrets"]:
        out.append(f"\n## ⚠ Suspected to be sensitiveInfo ({len(agg['secrets'])})")
    ...
    out.append(f"\n## endpoint/path ({len(agg['paths'])})")
    out += [f"  {p}" for p in agg["paths"][:120]]
    out.append(f"\n## Associated domain name ({len(agg['domains'])})")
    out += [f"  {d}" for d in agg["domains"][:60]]
    out.append(f"\n## absolute URL ({len(agg['urls'])})")
    out += [f"  {u}" for u in agg["urls"][:60]]
    return "\n".join(out)


# ── Unauthorized access detection (JS The collected interfaces are verified one by one)────────────────────────────────────

# destructiveAction: Even if you only send GET May also trigger side effects (text message bombardment/change data), alwaysSkipped
...
# Strong authentication wall signal: when it appears, it is judged as login/Block pages (avoid including "login" The public page of the navigation link is misjudged)
_AUTHWALL_MARKERS = (
    "Please log in", "Please firstLog in", "Not logged in", "unauthorized", "No permission", "Insufficient permissions", "Log in to view",
...
def _parse_auth_header(raw: Any) -> dict[str, str]:
    ...
    # naked token → as Bearer
    return {"Authorization": f"Bearer {text.strip()}"}


def _is_auth_wall(body: str) -> bool:
    """Is it a login?/Authentication interception page: strong copywriting signal or password existsInputFrame (not relying on naked login Type misjudgment)."""
    head = body[:4000]
    low = head.lower()
...
def _classify_unauth(status: int, body: str, ctype: str) -> tuple[str, bool]:
    """Return (Judgment copy, whethersuspected unauthorizedclue)."""
    if status in (401, 403):
        return "✓ Authenticated interception", False
    if status in (301, 302, 307, 308):
        return "↪ Jump(Suspected login)", False
    if status == 404:
        return "— does not exist", False
    if status == 405:
        return "· The method is notAllow", False
    if status == 200:
        if not body.strip():
            return "· 200 Empty response", False
        if _is_auth_wall(body):
            return "· 200 Log in/Authentication wall", False
        is_data = ("json" in ctype.lower()) or body.lstrip()[:1] in ("{", "[")
        if is_data:
            return "⚠ suspected unauthorized(returns data)", True
        if "html" in ctype.lower() or body.lstrip()[:1] == "<":
            return "· 200 HTML page(non-interface)", False  # Public page, non-interface is not authorized
        return "⚠ 200 Requires laborConfirm", True
...
async def _probe_endpoints(
    ...
    for ep in endpoints:
        ...
        if _host_of(full) != base_host:  # Do not hit associated domain names that are not within the authorized scope
            continue
        if _DESTRUCTIVE_RE.search(full):  # Read and write separation red line:Skippeddestructive interface
            results.append({"url": full, "status": "-", "verdict": "🚫 Skipped(destructive interface)", "lead": False, "length": 0})
            continue
        if full in seen:
            continue
        seen.add(full)
    ...
    # REST CRUD list/query/search Endpoints typically require POST(Including frame variants such as listForLayUI)
    ...
    async def one(url: str) -> None:
        async with sem:
            # priority GET;right REST CRUD list/query Endpoint extra attempts POST
            methods = ["GET"]
            ...
            for method in methods:
                try:
                    ...
                    if best_row is None:
                        best_row = {"url": url, "status": "ERR", "verdict": f"askFailed:{e}",
                ...
                if lead and auth:
                    try:
                        ...
                        if ra.status_code == 200 and abs(len(ra.content) - len(r.content)) <= max(50, len(r.content) * 0.1):
                            row["verdict"] = "🔴 UnauthorizedConfirm(nonetoken=havetoken)"
                    except Exception:
                        pass
                # reserveFindingThe method with stronger clues
    ...
    # Leads first、Press againStatusSort
    results.sort(key=lambda x: (not x.get("lead"), str(x.get("status"))))
    return results


async def execute_unauth_test(agent: Any, args: dict[str, Any]) -> str:
    """Perform unauthorized access detection on a batch of interfaces one by one (security only GET,Skippeddestructive interface)."""
    cfg = _get_recon_cfg(agent)
    ...
    if not base and endpoints:
        base = endpoints[0]
    if not base:
        return "[!] unauth_test need base_url(or in endpoints Mediumgive complete URL)"
    if "://" not in base:
        base = "http://" + base
    host = _host_of(base)
    ...
    if violation:
        return violation
    if not endpoints:
        return "[!] unauth_test need endpoints(interfacePath/URL list, usually from js_recon)"
    ...
    try:
        ...
        return f"[!] unauth_test implementError: {e}"

    leads = [r for r in rows if r.get("lead")]
    out = [f"# Unauthorized access detection — {host}  detection {len(rows)} interface, suspected clue {len(leads)}"]
    if auth:
        out.append("  (Enabled have/none token Differential comparison)")
    for r in rows:
        st = r.get("status")
    ...
    if leads:
        out.append("\n⚠ Focus on manual review (ConfirmCan I read other people’s data?/Sensitive or not):")
        out += [f"  {r['url']}" for r in leads]
    return "\n".join(out)


# ── Directory Enumeration(Reference dirsearch)──────────────────────────────────────────────────
...
async def execute_dir_enum(agent: Any, args: dict[str, Any]) -> str:
    """Directory Enumeration: Concurrent dictionary blasting, with 404 baseline / Global camouflage recognition andStatuscodeFilter."""
    cfg = _get_recon_cfg(agent)
    base = str(args.get("url", "") or "").strip()
    if not base:
        return "[!] dir_enum need url parameter"
    if "://" not in base:
        base = "http://" + base
    base = base.rstrip("/") + "/"
    host = _host_of(base)
    ...
    try:
        async with _make_client(cfg) as client:
            # 404 baseline + Global masquerading identification: request randomly does not existPath
            baseline_len = None
            try:
                rnd = await client.get(urljoin(base, "bughunter_nope_8f3a2c1e9b/"))
                if rnd.status_code in (200, 301, 302):
                    baseline_len = len(rnd.text)
                    # randomPathactuallyReturn 200 → Global camouflage response, stop blasting (CLAUDE.md iron law)
                    if rnd.status_code == 200:
                        return (
                            f"[!] dir_enum Termination: randomPath {base}bughunter_nope_... Return 200"
                            f"(length {baseline_len}),TargetSuspected of anyPathReturn 200, directory blasting is meaningless."
                        )
            except Exception:
                pass
            ...
            async def probe(path: str) -> None:
                ...
                if code in _HIT_CODES:
                    if baseline_len is not None and code in (200, 301, 302) and length == baseline_len:
                        return  # The same length as the camouflage baseline is judged as noise.
                    hits.append((code, length, path))

            await asyncio.gather(*(probe(p) for p in candidates))
    except Exception as e:
        return f"[!] dir_enum implementError: {e}"

    hits.sort(key=lambda x: (x[0], -x[1]))
    out = [f"# Directory Enumeration — {base}  ask {len(candidates)} strip,command {len(hits)}"]
