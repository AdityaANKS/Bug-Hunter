"""Bug Hunter Knowledge Retriever — retrieve relevant knowledge for the agent.
...
class _ChromaRetriever:
    ...
    def _build(self) -> None:  # pragma: no cover - requires chromadb installed
        ...
        self._collection = client.get_or_create_collection("bughunter_kb")
...
class KnowledgeRetriever:
    ...
    def _init_backend(self) -> None:
        ...
        if CHROMADB_AVAILABLE:
            try:
                ...
                if backend.has_data():
                    ...
                    self._status_detail = "ChromaDB Semantic retrievalEnabled"
                    return
                ...
                self._status_detail = f"ChromaDB initializationFailed: {exc}"
        ...
        try:
            ...
            self._status_detail = f"Keyword search initializationFailed: {exc}"
            return
        ...
        if keyword.has_data():
            ...
            if not CHROMADB_AVAILABLE:
                self._status_detail = (
                    ...
                    "Has been downgraded to keyword search"
                )
            elif not self._status_detail:
                self._status_detail = "Has been downgraded to keyword search"
        else:
            self._status = RetrieverStatus.DISABLED
            self._status_detail = "Knowledge baseEmpty, no data available"
