"""Textual-powered TUI workbench for Bug Hunter."""

# [Added] 2026-06-10
# Purpose: Build a modern fullscreen TUI Workbench using the Textual framework, replacing the old Rich number menu loop
# Implementation:
#   - CommandPalette: slash Ambiguous commandSearch + Keyboard navigation panel
#   - SecondaryPopup: auxiliary info popup (history snapshots/diagnostic reports)
#   - DashboardScreen: main navigation dashboard, integrating command input and status bar
#   - BugHunterApp: Textual App entry point, CSS themed styling
#   - run_tui_textual(): Event loop entry point, supports auto-reload configuration after launch
...
import bughunter.cli.tui as _tui

# [Added] 2026-06-10 Nyaecho - Natural language driven / responsive sidebar: Added color constants and action helper function imports
...
def _dispatch(session: dict[str, Any], text: str) -> str | None:
    ...
    # [Modified] 2026-06-10 Nyaecho - Fix IndexError caused by empty parts
...
def _h_scope(session: dict[str, Any], args: str) -> str | None:
    # [Modified] 2026-06-10 Nyaecho - Fix /scope port validation issue, add port validation to prevent ValueError
    state = session["state"]
    if args:
...
def _h_start(session: dict[str, Any], args: str) -> str | None:
    ...
    if args in ("-f", "--force"):
        return "launch"
    # [Added] 2026-06-10 Nyaecho - TUI natural language driven: /run <text> uses text as NL prompt to directly launch
    if args:
...
def _h_config(session: dict[str, Any], args: str) -> str | None:
    # [Modified] Refactor config flow: select provider → input API key → fetch model list → select/input model
    ...
    def on_apikey(v):
        ...
        # Skip fetching for custom provider or missing base_url, directly input manually
        if not base_url or not api_key:
            ...
            return
        # Show loading, fetch model list in background
...
def _h_sandbox(session: dict[str, Any], args: str) -> str | None:
    ...
    if not args or args.lower() == "status":
        ...
        if not sessions:
            _set_prompt(session, "message", "No sandbox sessions. Use /sandbox start to launch one.")
        else:
            lines = []
            for s in sessions:
                lines.append(f"{s.session_id}: {s.status} (port {s.ssh_port})")
            _set_prompt(session, "message", "\n".join(lines))
    elif args.lower() == "start":
        try:
            sess = asyncio.run(manager.start_sandbox(_SANDBOX_SESSION_ID))
            if sess.status == "running":
                _set_prompt(session, "message", f"Sandbox started: {sess.container_name} (SSH port {sess.ssh_port})")
            else:
                _set_prompt(session, "message", f"Sandbox failed to start (status: {sess.status})")
        except Exception as e:
            _set_prompt(session, "message", f"Error starting sandbox: {e}")
    elif args.lower() == "stop":
        try:
            ok = manager.stop_sandbox(_SANDBOX_SESSION_ID)
            _set_prompt(session, "message", "Sandbox stopped." if ok else "No running sandbox found.")
        except Exception as e:
            _set_prompt(session, "message", f"Error stopping sandbox: {e}")
    else:
        ...
        try:
            ...
            if result:
                ...
                _set_prompt(session, "message", f"$ {args}\n[exit {result.exit_code}] {output}")
            else:
                _set_prompt(session, "message", "Sandbox command failed.")
        except Exception as e:
            _set_prompt(session, "message", f"Sandbox exec error: {e}")
    return None


@_register_handler("findings")
def _h_findings(session: dict[str, Any], args: str) -> str | None:
    """Show findings info in the TUI."""
    _set_prompt(session, "message", _("tui.slash_findings") + "\n(Use 'findings' in the REPL for a detailed table.)")
    return None


@_register_handler("pool")
def _h_pool(session: dict[str, Any], args: str) -> str | None:
    """Show the model pool in the TUI."""
    config = session["config"]
    pool = config.llm.model_pool
    if not pool:
        _set_prompt(session, "message", f"No model pool configured. Current model: {config.llm.model}")
        return None
...
def _h_continue(session: dict[str, Any], args: str) -> str | None:
    if session.get("_last_draft") is not None:
        # [Modified] 2026-06-10 Nyaecho - Plan A cumulative concat: /continue [text] appends to _nl_history, reuses if no args
...
class DashboardScreen(Screen):
    ...
    def __init__(self, session: dict[str, Any]):
        ...
        # [Added] 2026-06-10 Nyaecho - Status bar message auto-hide: use incrementing counter to distinguish old/new message timers
        self._bar_msg_id: int = 0

    def compose(self) -> ComposeResult:
        # [Modified] 2026-06-10 Nyaecho - Responsive column layout: #output-log importation Horizontal #exec-row and #exec-sidebar side by side
    ...
    def _set_bar(self, text: str = "", style: str = "") -> None:
        # [Modified] 2026-06-10 Nyaecho - Status bar message auto-hide after 4s: msg_id counterprevent old timers from erroneously clearing new messages
        self._bar_msg_id += 1
    ...
    def _dismiss_bar(self, msg_id: int) -> None:
        # [Added] 2026-06-10 Nyaecho - Only close status bar when current message ID matches, prevent old timers from overriding new messages
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if text.startswith("/"):
            ...
            if result == "quit":
                ...
                # [Modified] 2026-06-10 Nyaecho - Pass NL text to subprocess, Carry accumulated NL history for /continue
            ...
            # [Added] 2026-06-10 Nyaecho - TUINatural language driven: Plain text without slash prefix directly launches as NL prompt
            state = self._s["state"]
            if not state.target.strip():
    ...
    def _start_execution(self, draft: Any = None, *, continuing: bool = False, nl_text: str | None = None) -> None:
        # [Modified] 2026-06-10 Nyaecho - Responsive column layout: Hide dashboard, show exec-row, Dynamically show/hide sidebar based on terminal width
        ...
        self._proc = None
        # [Added] 2026-06-10 Nyaecho - Show sidebar based on terminal width (shown when width >= 100 columns)
    ...
    def _run_subprocess(self, draft: Any, nl_text: str | None = None) -> None:
        # [Modified] 2026-06-10 Nyaecho - Pass NL text to CLI subprocess via --prompt
        import subprocess
        import sys

        from bughunter.cli.tui import build_command_preview_args

        cmd_args = build_command_preview_args(draft, nl_text=nl_text)
        args = [sys.executable, "-m", "bughunter.cli.main"] + cmd_args[1:]
    ...
    def _apply_responsive_layout(self) -> None:
        # [Added] 2026-06-10 Nyaecho - Responsive columns: show sidebar summary when terminal width >= 100 columns, otherwise only show output log
        threshold = 100
    ...
    def _check_responsive_resize(self) -> None:
        # [Added] 2026-06-10 Nyaecho - Check terminal width changes every 0.3s during execution, dynamically switch column mode
        if not self._worker_running:
            return
        self._apply_responsive_layout()
    ...
    def _start_polling(self) -> None:
        self._poll_output()
        # [Added] 2026-06-10 Nyaecho - Check terminal width changes during round polling for responsive columns
    ...
    def _build_exec_sidebar(self) -> str:
        # [Added] 2026-06-10 Nyaecho - Build execution sidebar summary: Target/Mode/Scope/Allow-Block/Resume/LLM Info
        state = self._s["state"]
        mode = _tui.MODES[state.mode]
...
# [Modified] 2026-06-10 Nyaecho - Added #exec-row split container + #exec-sidebar 30-column sidebar, supports split display when terminal width >= 100 columns
CSS = """
#body {
    height: 1fr;
    overflow-y: auto;
