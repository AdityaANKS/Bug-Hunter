"""TUI helpers for the Bug Hunter CLI."""

# [Modified] Major refactor: from Rich number menu driven to prompt_toolkit + slash command system
# - Added opencode-style color palette (C_PRIMARY / C_SECONDARY wait)
# - Added slash command system (/target /mode /scope /start /config wait)
# - Added prompt state machine (input / choice / confirm / chain)
# - Added _run_pt_tui function providing prompt_toolkit app main loop
# - Old Rich Prompt preserved in _prompt_* functions as compatibility fallback
# - Original run_tui() changed to bridge to tui_textual.run_tui_textual()
...
# [Modified] 2026-06-10 Nyaecho - Move prompt_toolkit import to _run_pt_tui() inside function to avoid hard dependencies
...
from bughunter.i18n import _, init_i18n
from bughunter.target_state.store import get_target_state_preview, list_target_snapshots

# ── opencode-inspired colour palette ──
# [Modified] Replace Rich default colors with unified color variables, for easy future theme switching
...
def build_dashboard(config, state: TuiState) -> Group:
    """Build the first-screen Bug Hunter TUI dashboard."""
    ...
    title = Text(" Bug Hunter TUI", style=f"bold {C_ACCENT}")
    ...
    # [Modified] Use Rich Text with per-segment coloring,to avoid displaying markup as plain strings
    footer_body = Text()
...
def run_tui(
    *,
    ...
    # [Modified] Original Rich main loop replaced with Textual backend, bridging to tui_textual.run_tui_textual()
    from bughunter.cli.tui_textual import run_tui_textual
...
def _run_pt_tui(session: dict[str, Any]) -> Optional[str]:
    ...
    """
    # [Modified] 2026-06-10 Nyaecho - Move prompt_toolkit import toinside function to avoid hard dependencies
...
# [Modified] Added prompt state machine supporting input/choice/confirm/chain four interactive modes
# Replacing Rich's Prompt.ask/Confirm.ask, deeply integrated with prompt_toolkit
...
# [Modified] Added slash command system replacing number menu, with completion and hotkey support
# Command mapping: /target→Original1, /mode→Original2, /scope→Original3, /start→Original4...
# /history→Original5, /report→Original6, /diag→Original7, /config→Original8, /quit→Originalq
...
def _build_slash_commands() -> dict[str, str]:
    ...
    return {
        ...
        "run": _("tui.slash_run"),
        # [Added] 2026-06-10 Nyaecho - TUI command palette added /continue slash command entry
        ...
        "sandbox": _("tui.slash_sandbox"),
        "findings": _("tui.slash_findings"),
        "pool": _("tui.slash_pool"),
        "budget": _("tui.slash_budget"),
...
def _cmd_report(session: dict[str, Any], args: str) -> None:
    ...
    from bughunter.cli.main import _generate_report_for_target
...
def _cmd_config(session: dict[str, Any], args: str) -> None:
    ...
    def _on_provider(value: str) -> None:
        if value and value != current_provider:
            ...
            config = session["config"]
        # Flow change: input API key first after selecting provider
    ...
    def _on_apikey(value: str) -> None:
        ...
        # custom Provideror missing base_url/api_key hourSkippedObtain directly manuallyInput
        if not base_url or not api_key:
            ...
            return
        # prompt_toolkit Version: Get the model list synchronously
        _set_prompt_message(session, _("tui.fetching_models"))
        # Note: exist prompt_toolkit synchronous loopMedium, the message will not be rendered immediately
        # Get the model list directly and synchronously
...
def _cmd_sandbox(session: dict[str, Any], args: str) -> None:
    ...
    if not args or args.lower() == "status":
        ...
        if not sessions:
            _set_prompt_message(session, "No sandbox sessions. Use /sandbox start to launch one.")
        else:
            lines = []
            for s in sessions:
                lines.append(f"{s.session_id}: {s.status} (port {s.ssh_port})")
            _set_prompt_message(session, "\n".join(lines))
    elif args.lower() == "start":
        try:
            sess = asyncio.run(manager.start_sandbox(_SANDBOX_SESSION_ID))
            if sess.status == "running":
                _set_prompt_message(session, f"Sandbox started: {sess.container_name} (SSH port {sess.ssh_port})")
            else:
                _set_prompt_message(session, f"Sandbox failed to start (status: {sess.status})")
        except Exception as e:
            _set_prompt_message(session, f"Error starting sandbox: {e}")
    elif args.lower() == "stop":
        try:
            ok = manager.stop_sandbox(_SANDBOX_SESSION_ID)
            _set_prompt_message(session, "Sandbox stopped." if ok else "No running sandbox found.")
        except Exception as e:
            _set_prompt_message(session, f"Error stopping sandbox: {e}")
    else:
        # Execute command in sandbox
        try:
            asyncio.run(manager.start_sandbox(_SANDBOX_SESSION_ID))
            result = asyncio.run(manager.execute_command(_SANDBOX_SESSION_ID, args, timeout=60))
            if result:
                output = result.stdout or result.stderr or "(no output)"
                _set_prompt_message(session, f"$ {args}\n[exit {result.exit_code}] {output}")
            else:
                _set_prompt_message(session, "Sandbox command failed.")
        except Exception as e:
            _set_prompt_message(session, f"Sandbox exec error: {e}")


@_register_handler("findings")
def _cmd_findings(session: dict[str, Any], args: str) -> None:
    """Handle /findings slash command."""
    _set_prompt_message(session, _("tui.slash_findings") + "\n(Findings are displayed in the main REPL session. Use 'findings' in the REPL for a detailed table.)")


@_register_handler("pool")
def _cmd_pool(session: dict[str, Any], args: str) -> None:
    """Handle /pool slash command."""
    config = session["config"]
    pool = config.llm.model_pool
    if not pool:
        _set_prompt_message(session, f"No model pool configured. Current model: {config.llm.model}")
        return
    lines = [f"Model Pool ({len(pool)} models):"]
    for entry in sorted(pool, key=lambda e: e.tier):
        status = "enabled" if entry.enabled else "disabled"
        lines.append(f"  T{entry.tier} {entry.name} ({entry.model}) [{entry.role}] {status}")
    enabled = [e for e in pool if e.enabled]
    if enabled:
        primary = sorted(enabled, key=lambda e: e.tier)[0]
        lines.append(f"\nActive primary: {primary.name} ({primary.model})")
    _set_prompt_message(session, "\n".join(lines))


@_register_handler("budget")
def _cmd_budget(session: dict[str, Any], args: str) -> None:
    """Handle /budget slash command."""
    config = session["config"]
    budget_arg = args.strip().lower()
    if not budget_arg:
        current = getattr(config.session, "scan_time_budget", 0)
        if current > 0:
            _set_prompt_message(session, f"⏱️ Time budget: {current} min (Quick {int(current*0.2)}m, Standard {int(current*0.4)}m, Deep {int(current*0.3)}m, Report {int(current*0.1)}m)")
        else:
            _set_prompt_message(session, "⏱️ Time budget: unlimited. Use /budget <minutes> to set.")
    elif budget_arg == "off" or budget_arg == "0":
        config.session.scan_time_budget = 0
        _set_prompt_message(session, "⏱️ Time budget disabled (unlimited).")
    elif budget_arg.isdigit():
        mins = int(budget_arg)
        config.session.scan_time_budget = mins
        _set_prompt_message(session, f"⏱️ Time budget set to {mins} min (Quick {int(mins*0.2)}m, Standard {int(mins*0.4)}m, Deep {int(mins*0.3)}m, Report {int(mins*0.1)}m)")
    else:
        _set_prompt_message(session, "Invalid budget. Use /budget <minutes> (e.g. /budget 30) or /budget off.")

# ── (kept for backward compatibility) ──
# [Modified] The following old Rich/Prompt Functions reserved for testing and CLI call directly, New code should use slash command system
...
def build_runtime_diagnostic(config) -> TuiRuntimeDiagnostic:
    ...
    try:
        ...
        return TuiRuntimeDiagnostic(
            ...
            model=model,
            api_key_configured=api_key_configured,
            mcp_error=f"MCP diagnosisFailed: {exc}",
        )
...
def build_command_preview_args(draft: TuiTaskDraft, nl_text: str | None = None) -> list[str]:
    ...
    # [Modified] 2026-06-10 Nyaecho - TUINatural language driven: support nl_text incoming and through --prompt passed toCLIchild process
    # [Modified] 2026-06-10 Nyaecho - Add security riskHints: Passed via command linepromptMay be exposed to other local users
    args = ["bughunter", draft.command, draft.target]
    if nl_text:
...
def _prompt_llm_config(screen: Console, config):
    ...
    # Process changes: FirstInput API Key, and then get the model list
...
def _default_launcher(draft: TuiTaskDraft) -> None:
    from bughunter.cli import main as cli_main
