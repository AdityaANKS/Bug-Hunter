"""Bug Hunter MCP Lifecycle Manager — start/stop MCP servers and manage their lifetime."""
...
class MCPLifecycleManager:
    ...
    def __init__(self, config: BugHunterConfig) -> None:
        self.config = config
    ...
    def _start_server(self, name: str, config: MCPServerConfig) -> bool:
        ...
        if transport.type in HTTP_TRANSPORT_TYPES:
            ...
            # detectionFailedFallback to static knownTool, at least let LLM can seeToolname
            if not attached:
                self._register_known_tools(name)
            return True
    ...
    async def _async_probe_http_server(
        ...
        try:
            ...
            # from ExceptionGroup MediumExtract the root cause (anyio TaskGroup The real exception is wrapped in a layer)
            detail = str(exc)
            if hasattr(exc, "exceptions"):
                subs = list(getattr(exc, "exceptions", []))
                if subs:
            ...
            if "already connected" in detail.lower():
                detail += " (Please restart MCP Serve or close old client connections)"
            return False, detail, []
    ...
    def _run_probe(
        ...
        When called from within an active event loop (e.g. BugHunter's REPL under
    ...
    async def _async_probe_stdio_server(
        ...
        try:
            async with stdio_client(server) as (read_stream, write_stream):
                # async with ClientSession start up _receive_loop,otherwise initialize() No one will get stuck reading the response.
    ...
    async def _get_or_create_persistent_stdio_session(self, server_name: str) -> Any:
        ...
        # Enter ClientSession context to start _receive_loop; Otherwise, subsequent calls cannot read the response and get stuck.
        try:
    ...
    async def _get_or_create_persistent_http_session(self, server_name: str) -> Any:
        ...
        # When connecting for the first timeFindingrealityToolAnd replace the static placeholder registered at startupTool
        try:
            tools = await session.list_tools()
            tool_defs = self._normalize_mcp_tools(getattr(tools, "tools", []) or [])
            if tool_defs:
    ...
    async def _dispatch_call_tool(
        ...
        try:
            ...
            # UniversalPath: any scripture SDK attach Successof stdio/streamable-http services (such asCustom
            # streamable-mcp-server) all follow the truthSessioncall instead of falling back to unsupported.
    ...
    async def _call_fetch(self, args: dict) -> str:
        ...
        try:
            ...
            return "[!] httpx Not installed, unable to execute fetch ask"
        except Exception as e:
            return f"[!] fetch askFailed: {e}"
    ...
    async def _call_memory(self, tool_name: str, args: dict) -> str:
        ...
        if tool_name == "save":
            ...
            return f"[+] alreadySave: {args.get('key', '')}"
