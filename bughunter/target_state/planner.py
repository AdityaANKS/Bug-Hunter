...
def build_resume_plan(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if pending_sorted:
        next_actions = [
            ...
            "Avoid re-running top-level directory enumeration",
        ]
        if violation_events:
            next_actions.append("Avoid actions and tool paths recently blocked by constraint policies")
        if blocked_targets:
            next_actions.append("Skip confirmed unreachable targets, focus on verifying still-accessible entry points")
        if low_value_rounds >= 3:
            next_actions.append("Too many consecutive low-value rounds, prioritize switching parameters or new entry points")
        if recon_priority_assets:
            next_actions.append(f"Prioritize high-value recon asset: {recon_priority_assets[0]}")
        return {
            "strategy": "verify_pending_findings",
            "reason": _build_reason(
                f"{len(pending_sorted)} pending vulnerability candidates exist, prioritize verification",
    ...
    if verified_sorted:
        next_actions = [
            "Prioritize expanding exploitation chains around verified vulnerabilities",
            "Avoid falling back to low-value basic reconnaissance",
        ]
        if violation_events:
            next_actions.append("Confirm constraint policies won't block exploitation before expanding")
        if current_attack_path:
            next_actions.append(f"Don't stay stuck on old path {current_attack_path}, prioritize new exploitation avenues")
        if recon_priority_assets:
            next_actions.append(f"Leverage high-value recon asset for exploitation: {recon_priority_assets[0]}")
        return {
            "strategy": "exploit_expand",
            "reason": _build_reason(
                f"{len(verified_sorted)} verified vulnerabilities exist, prioritize continued exploitation and expansion",
                blocked_targets=blocked_targets,
                low_value_rounds=low_value_rounds,
            ),
    ...
    if incomplete:
        ...
        if violation_events:
            next_actions.append("Prioritize reconnaissance actions not blocked by constraints")
        if blocked_targets:
            next_actions.append("Ignore unreachable sub-targets, prioritize completing recon on accessible assets")
        if recon_priority_assets:
            next_actions.append(f"Prioritize continuing with high-value recon asset: {recon_priority_assets[0]}")
        return {
    ...
    if low_value_rounds >= 3:
        next_actions.append("Avoid recently failed scan paths, switch to new entry points or different parameters")
    if recon_priority_assets:
        next_actions.append(f"Prioritize testing high-value recon asset: {recon_priority_assets[0]}")
    return {
        "strategy": "continue_scan",
        "reason": _build_reason(
            "No verified vulnerabilities yet, continue advancing scans from candidate attack surfaces",
            blocked_targets=blocked_targets,
            low_value_rounds=low_value_rounds,
        ),
        "recommended_phase": PentestPhase.VULN_DISCOVERY.value,
...
def _build_reason(base: str, *, blocked_targets: list[str], low_value_rounds: int) -> str:
    suffix: list[str] = []
    if blocked_targets:
        suffix.append(f"{len(blocked_targets)} unreachable target(s) exist")
    if low_value_rounds >= 3:
