...
from bughunter.config.settings import TARGETS_DIR, ensure_dirs
from bughunter.target_state.planner import (
    build_resume_plan,
...
def _build_resume_summary(raw: dict[str, Any], resume_meta: dict[str, Any]) -> str:
    ...
    if resume_meta.get("resume_strategy"):
        parts.append(f"- Resume strategy: {resume_meta['resume_strategy']}")
    ...
    if runtime_meta.get("current_attack_path"):
        parts.append(f"- Current attack path: {runtime_meta['current_attack_path']}")
    if recon_data:
        parts.append(f"- Recon data keys: {', '.join(sorted(recon_data.keys())[:10])}")
    if resume_meta.get("priority_targets"):
        parts.append(f"- Priority targets: {', '.join(resume_meta['priority_targets'][:5])}")
