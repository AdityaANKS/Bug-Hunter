...
class TaskOptions(BaseModel):
    ...
    prompt: Optional[str] = Field(
        default=None, description="User's natural language scan instructions for the AI agent"
    )
...
class ModelPoolEntryView(BaseModel):
    name: str
    provider: str
    model: str
    role: str
...
class ConfigView(BaseModel):
    ...
    model_pool: list[ModelPoolEntryView] = Field(default_factory=list)
...
SandboxStatusType = Literal["starting", "running", "stopped", "error", "not_found"]


class SandboxSessionView(BaseModel):
    """Public view of a sandbox session."""

    session_id: str
    container_id: str = ""
    status: SandboxStatusType = "starting"
    host: str = "127.0.0.1"
    ssh_port: int = 0
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat())
